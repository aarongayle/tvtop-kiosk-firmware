...
static bool want_ap;
static bool ap_keep_station;
static char ap_ssid[16];
static uint32_t seen_session;
static net_check_t check_result = NET_CHECK_IDLE;   // the modem boot we have already configured
...
static void send_ap(void) {
    ...
    buf[1 + sl] = ap_keep_station ? 1 : 0;
    modem_link_send(H_AP_START, buf, (uint16_t)(2 + sl));
}
...
static void dispatch(uint8_t type, const uint8_t *p, uint16_t len) {
    ...
    case M_NET_CHECK: {
        if (!len) return;
        static const net_check_t map[] = { NET_CHECK_ONLINE, NET_CHECK_CAPTIVE, NET_CHECK_NO_DNS, NET_CHECK_NO_ROUTE };
        check_result = p[0] < 4 ? map[p[0]] : NET_CHECK_NO_ROUTE;
        return;
    }
...
// The ESP runs AP and station together, so the fallback AP costs nothing: the kiosk keeps hunting
// for a network it knows while offering a way in.
...
void net_wifi_check_start(void) {
    if (!modem_link_ready()) { check_result = NET_CHECK_NO_ROUTE; return; }
    const char *url = MODEM_NET_CHECK_URL;
    uint16_t ul = (uint16_t)strlen(url);
    uint8_t hdr[2];
    memcpy(hdr, &ul, 2);
    if (modem_link_send2(H_NET_CHECK, hdr, 2, url, ul)) check_result = NET_CHECK_PENDING;
}

net_check_t net_wifi_check_result(void) { return check_result; }
